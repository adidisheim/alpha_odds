"""
Full V2 retrain with restricted features — exact replica of _03_win_probability_model_v2.py
but with feature subsetting from Phase 1 ranking.

Array job: task_id = 0..431
  feat_idx = task_id // 108    (0..3 -> feature counts [5, 10, 20, 30])
  grid_id  = task_id %  108 // 4
  t_def_idx= task_id %  4

Saves to: res/feature_importance_study/full_retrain/n{N}/t{T}/{hp_str}/

Usage:
    python _05_full_retrain.py <task_id>
"""
...
N_T_DEFS = len(T_DEFS)
N_GRID = 27  # 3*3*3
JOBS_PER_FEAT = N_GRID * N_T_DEFS  # 108


# ──────────────────────────────────────────────
# Cross-runner features (exact copy from V2)
# ──────────────────────────────────────────────
...
# ──────────────────────────────────────────────
# Feature normalizer (exact copy from V2)
# ──────────────────────────────────────────────
...
if __name__ == '__main__':
    ...
    # Decode task_id
    feat_idx = task_id // JOBS_PER_FEAT
    remainder = task_id % JOBS_PER_FEAT
    grid_id = remainder // N_T_DEFS
    t_def_idx = remainder % N_T_DEFS

    n_features_target = FEATURE_COUNTS[feat_idx]
    t_definition = T_DEFS[t_def_idx]

    grid_combs = get_grid_combinations()
    hp = grid_combs[grid_id]
    hp_str = f"ne{hp['n_estimators']}_md{hp['max_depth']}_lr{hp['learning_rate']}"

    print(f"=== Full Retrain: top-{n_features_target} features ===", flush=True)
    print(f"Task {task_id}: feat_idx={feat_idx}, grid_id={grid_id}, t_def={t_definition}", flush=True)
    print(f"HP: {hp} -> {hp_str}", flush=True)
    ...
    print(f"Dropped {n_before - len(df)} NaN rows", flush=True)
    ...
    # Always keep best_back_m0 and best_lay_m0 for edge calculation
    ...
    print(f"Using {len(predictors_col)} features (top-{n_features_target})", flush=True)

    # ── Train/Val/Test split (same as V2) ──
    oos_year = 2025
    val_year = 2024
    ...
    print(f"Final features: {len(predictors_col)}", flush=True)
    ...
    print(f"XGB best iter: {xgb_best_iter}/{hp['n_estimators']}", flush=True)
    ...
    # market_prob already set by add_cross_runner_features before normalization
    ...
    if 'marketBaseRate' in oos_originals.columns:
    ...
    for name, probs in models_to_eval.items():
        ...
        print(f"  {name:12s} LL: {ll:.6f}", flush=True)

    # ══════════════════════════════════════════════
    # Save — same structure as original V2
    # ══════════════════════════════════════════════
    save_dir = f'{Constant.RES_DIR}/feature_importance_study/full_retrain/n{n_features_target}/t{t_definition}/{hp_str}/'
    os.makedirs(save_dir, exist_ok=True)

    # save_df.parquet (same columns as original V2)
    ...
    metrics = {
        ...
        'n_features_target': int(n_features_target),
    ...
    print(f"RAM: {process.memory_info().rss / 1024**3:.2f} GB", flush=True)
