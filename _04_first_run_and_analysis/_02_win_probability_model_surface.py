"""
Win Probability Model — Surface-only variant (no q_100/q_1000 depth features).

Identical to _02_win_probability_model.py except:
  1. Removes all q_100 and q_1000 features from predictors
  2. Saves to res/win_model_surface/ instead of res/win_model/

Usage:
    python _02_win_probability_model_surface.py <grid_comb_id> <t_definition>

    grid_comb_id: index into the hyperparameter grid (0-based)
    t_definition: which time snapshot features to use (0, 1, 2, or 3)

Output saved to: res/win_model_surface/t{t_definition}/{grid_hash}/
"""
...
# ──────────────────────────────────────────────
# Hyperparameter grid (same as _02)
# ──────────────────────────────────────────────
GRID_PARAMS = {
...
# ──────────────────────────────────────────────
# Feature normalizer (reused from _02)
# ──────────────────────────────────────────────
...
if __name__ == '__main__':
    ...
    print(f"=== Win Probability Model V1 SURFACE ===", flush=True)
    ...
    # Add runner_position as feature
    ...
    # ── Preserve original prices for betting simulation ──
    ...
    save_dir = f'{Constant.RES_DIR}/win_model_surface/t{t_definition}/{hp_str}/'
